...
def update_attendance_in_db_in(present):
    today = date.today()  # Use date.today() without the 'datetime' prefix
    current_time = timezone.now()  # Use timezone.now() to get an aware datetime
    ...
    for person in present:
        try:
            ...
            print(f"User with username '{person}' does not exist.")
            # You might want to log an error, create a new user, or handle it in another way
        ...
        if present[person]:
            ...
            # Create a new time record for login
            a = Time(user=user, date=today, time=current_time, out=False)
            a.save()

            # Calculate the logout time and create a time record for logout
            logout_time = current_time + timedelta(minutes=2)  # Adjust the duration as needed
            a = Time(user=user, date=today, time=logout_time, out=True)
            a.save()
...
def hours_vs_student_given_date(present_qs,time_qs):
	...
	rcParams.update({'figure.autolayout': True})
	plt.tight_layout()
	plt.savefig('./recognition/static/recognition/img/attendance_graphs/hours_vs_students/1.png')
	plt.close()
	return qs
...
def students_present_today():
    today = date.today()  # Use date.today() as a method
    qs = Present.objects.filter(date=today).filter(present=True)
    return len(qs)
...
def this_week_std_count_vs_date():
    today = date.today()
    ...
    loop_date = monday_of_this_week  # Use a different variable name

    while cnt < 5:
        date_str = str(loop_date)
        str_dates_all.append(date_str)
        
        if str_dates.count(date_str) > 0:
            idx = str_dates.index(date_str)
            std_cnt_all.append(std_count[idx])
        else:
            std_cnt_all.append(0)

        loop_date += timedelta(days=1)  # Increment the loop_date variable
        cnt += 1

    df = pd.DataFrame()
    df["date"] = str_dates_all
    df["Number of students"] = std_cnt_all
...
def last_week_std_count_vs_date():
    today = date.today()
    some_day_last_week = today - timedelta(days=7)
    monday_of_last_week = some_day_last_week - timedelta(days=(some_day_last_week.isocalendar()[2] - 1))
    monday_of_this_week = monday_of_last_week + timedelta(days=7)
    ...
    for obj in qs:
        date_obj = obj.date  # Use a different variable name here
        str_dates.append(str(date_obj))
        qs = Present.objects.filter(date=date_obj).filter(present=True)
        std_count.append(len(qs))

    while cnt < 5:
        loop_date = str(monday_of_last_week + timedelta(days=cnt))
        cnt += 1
        str_dates_all.append(loop_date)
        if str_dates.count(loop_date) > 0:
            idx = str_dates.index(loop_date)
            std_cnt_all.append(std_count[idx])
        else:
            std_cnt_all.append(0)
...
from django.utils import timezone
from datetime import datetime, timedelta, date  # Import date from datetime module
from django.contrib.auth.models import User
#from .models import Present, Time  # Import your models
import dlib
import cv2
import imutils
from imutils.video import VideoStream
...
# Assuming your 'update_attendance_in_db_in' function is defined somewhere in your code
# If not, make sure it is defined and imported appropriately
        ...
        for face in faces:
            ...
            if pred != [-1]:
                ...
                if count[pred] == 4 and (time.time() - start[pred]) > 1.2:
                    ...
                    present[pred] = True
                    log_time[pred] = timezone.now()  # Use timezone.now() to get an aware datetime
                    count[pred] = count.get(pred, 0) + 1
...
def view_attendance_home(request):
	total_num_of_std=total_number_students()
	std_present_today=students_present_today()
	this_week_std_count_vs_date()
	last_week_std_count_vs_date()
	return render(request,"recognition/view_attendance_home.html", {'total_num_of_std' : total_num_of_std, 'std_present_today': std_present_today})

from .forms import DateForm  # Make sure to import your DateForm
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if len(time_qs) > 0 or len(present_qs) > 0:
                ...
                messages.warning(request, f'No records for the selected date.')
...
def class_dashboard(request):
             ...
             return render(request, 'recognition/class_dashboard.html')
from django.shortcuts import render, redirect
from django.contrib import messages
from django.contrib.auth.decorators import login_required
from users.models import UserProfile  # Adjust the import based on your app structure
import datetime
...
import datetime
from users.models import UserProfile, Present, Time

@login_required
def update_class_selection(request, class_name):
    valid_class_names = ["Calculus", "Digital electronics", "Mobile Application", "Green Computing"]

    if class_name not in valid_class_names:
        messages.error(request, "Invalid class selection.")
        return redirect('your_units_registered_url') 

    user_profile, created = UserProfile.objects.get_or_create(user=request.user)
    user_profile.class_column = class_name
    user_profile.save()

    present_entry, created = Present.objects.get_or_create(user=request.user, date=datetime.date.today())
    present_entry.present = True
    present_entry.class_name = class_name
    present_entry.save()

    time_entry, created = Time.objects.get_or_create(user=request.user, date=datetime.date.today())
    time_entry.time = datetime.datetime.now()
    time_entry.class_name = class_name
    time_entry.save()

    return redirect('home')  # Replace with the actual URL name
